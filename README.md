...
### النشر
- **Vercel** للاستضافة مع API Proxy
- **PWA** للتطبيق الموبايل
- **Service Worker** للعمل بدون إنترنت
- **API Proxy** لحل مشاكل CORS

## 📋 المتطلبات

### قبل البدء
1. حساب Google لإنشاء Google Sheets
2. حساب Vercel للاستضافة (مطلوب للـ API Proxy)
...
- **Vercel مطلوب** للـ API Proxy (حل مشاكل CORS)
...
## 🌐 API Proxy - حل مشاكل CORS

### كيف يعمل الـ API Proxy؟
التطبيق يستخدم **API Proxy** في Vercel لحل مشاكل CORS مع Google Apps Script:

#### المشكلة التي يحلها:
- **CORS Errors**: المتصفحات تحجب الطلبات المباشرة إلى Google Apps Script
- **Security**: منع الوصول المباشر للـ API من المتصفح

#### كيفية العمل:
1. **التطبيق** يرسل الطلبات إلى `/api/proxy` في Vercel
2. **Vercel Proxy** يحول الطلبات إلى Google Apps Script
3. **Google Apps Script** يعالج الطلب ويرد
